...
- **33-page hybrid MPA/SPA** (vanilla HTML/CSS/JS). Works as MPA over `http://` (serve `dist/`) and as a **single-file SPA** (`dist/index.standalone.html`) over `http://`, `file://` and Android `content://` — all navigation is hash-based (`#/path`), so **no web server or sibling files are required to browse the whole site**.
- **Content:**
  ...
  - **3 learn pages** in `19-learn.js`: `learn/getting-started`, `learn/glossary`, `learn/references` (30 → **33 pages**).
- Build sizes: `dist/script.js` 634 KB, `dist/index.standalone.html` 695 KB, 47 static HTML pages.
- `validate.js` reports: **33 pages, ALL CHECKS PASSED** (205 explain blocks, 2738 explained lines).
...
4. **Stale test suites updated** in `/tmp/opencode/`: page count 30 → 33, home cards `#article .card` (26) → `#article .ds-card` (15, `DS_CARDS`), sidebar `#topics-children` → `#array-children`, coming-soon paths excluded from the DB-key check (they live in `DS_CARDS`).
...
```
`validate.js` must report: 33 pages, ALL CHECKS PASSED. All 5 suites must pass (last run: app ✓, dom ✓, android ✓, standalone ✓, mpa ✓).
